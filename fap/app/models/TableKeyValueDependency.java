...
// === MANUAL REGION START ===
	...
	//Prefijo para identificar la cache
		 ...
		 * 		CACHEPREFIXLIST + table  : Lista de TableKeyValueDependency
		 * 		CACHEPREFIXMAP  + table  : Map<key,dependency> para hacer las búsquedas más rapidas
		 * @param table
		 */
		public static void renewCache(String table){
			...
			for(TableKeyValueDependency tkv : list){
				m.put(tkv.key, tkv.dependency);
			}
		...
		public static void removeDependency(TableKeyValueDependency tkv){
		...
		public static void deleteTable(String table){
			deleteCache(table);
			TableKeyValueDependency.delete("table = ?", table);
		}
		 ...
		 * @return
		 */
		public static List<String> keys(List<TableKeyValueDependency> tabla){
			if(tabla == null)
				return null;
			
			List<String> keys = new ArrayList<String>();
			for(TableKeyValueDependency tkv : tabla){
				keys.add(tkv.key);
			}
			return keys;
		}
	    ...
	    public static long loadFromFiles(){
	        ...
	        List<TableKeyValueDependency> all = TableKeyValueDependency.findAll();
	        HashSet<String> tables = new HashSet<String>();
	        for (TableKeyValueDependency keyDependency: all){
	        	tables.add(keyDependency.table);
	        }
	        for (String table: tables){
	        	TableKeyValueDependency.renewCache(table);
	        }
			
	        return TableKeyValueDependency.count();
	    }


		@Override
		public String toString() {
			return "TableKeyValueDependency [table=" + table + ", key=" + key + ", dependency="
					+ dependency + ", id=" + id + "]";
		}
