...
Los permisos se podrán aplicar a páginas, grupos, forms, tablas, al menú (tanto grupos como elementos individuales). Los permisos que se aplican a las tablas sirven para filtrar las filas que van a aparecen al renderizarla. Para más información sobre como funcionan los permisos, "consultar esta página":curso-permisos.
...
*  **action** : Accion para la que se está consulta el permiso (leer, editar, crear, borrar).
*  **_permiso** : Modo de presentación para el que se está consultando el permiso (editable, visible, none).
...
bc. Permiso listaSolicitudes{
	...
	when
		(accion = leer and agente.rolActivo.nombre = "administrador")
		or
		(
			accion = leer
			and 
