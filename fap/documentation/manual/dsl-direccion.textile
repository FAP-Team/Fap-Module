...
Permite indicar que la dirección será elegible por el propio solicitante, mediante un combo 'Tipo Dirección'. Los valores de este combo estarán compuestos por los valores parseados de una property situada en el application.conf. Esta property es:

* *fap.direcciones.tipo=* _canaria,nacional,internacional_. Por defecto tiene los valores que vemos (todos los posibles), si se quiere personalizar dicho combo habrá que sobreescribir la property en nuestro application.conf de la aplicación e indicarle que valores queremos seleccionar.
...
Otros valores que se pueden rellenar son: localidad, código postal, calle, número, otros (puerta,etc.).

h3. Dirección Nacional

Supone que el país será España. A partir de ahí deja seleccionar la comunidad autónoma que queramos, y dependiendo de ésta, su provincia. Dependiendo de esta provincia podremos seleccionar un municipio.

Otros valores que se pueden rellenar son: localidad, código postal, calle, número, otros (puerta,etc.).

h3. Dirección Internacional

Permite seleccionar un País, de la lista de todos los países del mundo.

Además, tiene campos distintos a las otras direcciones, como el Estado, y no tiene campos como Comunidad Autónoma, Provincia y Municipio.
