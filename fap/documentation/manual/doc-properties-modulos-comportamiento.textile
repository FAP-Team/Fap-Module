...
| Configurar que las entidades se guarden nada más invocarlas con el Nuevo de las tablas. | *fap.entidades.guardar. antes* | _[true / false]_ | _fap.entidades.guardar.antes =false_ (Valor por defecto que trae el módulo FAP) |
...
| Para que se validen y guarden los datos de la página que contiene una tabla que redirige a otra página antes de ir a esa otra página. | *fap.tablas.validacion. antesRedirigirPaginas* | _[true/false]_ |  fap.tablas.validacion. antesRedirigirPaginas=true |
...
| Tipo de documento de la solicitud en evaluacion, que pueden ver los evaluadores | *fap.baremacion. evaluacion.documento. solicitud* | _String_ | fap.baremacion.evaluacion. documento.solicitud =eadmon://gobcan.es/ tiposDocumentos/ TDC000000000000000004/v01
...
| Tipo de direcciones usadas en el combo del elemento dirección | *fap.direcciones.tipo* | _tiposDirecciones_ | _fap.direcciones.tipo =canaria, nacional, internacional_ (Valor por defecto que trae el módulo FAP) |
| Validación y guardado de la página antes de abrir la página de una tabla | *fap.tablas.validacion. antesRedirigirPaginas* | _[true/false]_ | _fap.tablas.validacion. antesRedirigirPaginas =true_ (Valor por defecto que trae el módulo FAP, a partir de la versión 2.1, antes _false_) |
...
| Para habilitar la alerta cuando te rediriges a otra página y has modificado campos que no has guardado | *fap.javascript. detectarcambios. noguardados* | _[true/false]_ | fap.javascript. detectarcambios. noguardados =true |
| Para habilitar la tabla de documentación externa | *fap.documentacion. documentosExternos* | _[true/false]_ | fap.documentacion. documentosExternos=true |
