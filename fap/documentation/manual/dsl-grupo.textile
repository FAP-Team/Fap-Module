...
* **mostrarSiCheck _[check]_ = true | false** (opcional) : Muestra el grupo si el valor del check especificado es "true" o "false".
* **mostrarSiRadioBooleano _[radioBooleano]_ = true | false** (opcional) : Muestra el grupo si el valor del radioBooleano especificado es "true" o "false". False significa que está activo el botón izquierdo, y true que es el botón de la derecha el que está presionado.
...
* **mostrarSiExpresion _[string]_** (opcional) : Muestra el grupo si el valor de la expresión que se especifique es verdadero.
...
* **borde = true | false** (opcional) : Si es false no se mostrará el cuadro que engloba a los elementos del grupo.
...
Permite mostrar el grupo si la expresión que se le asigna es verdadera (Ej: _"4 == 4", "true", ...) o un valor válido para el *if* de los templates de play ("Ver aquí":http://www.playframework.org/documentation/1.2.7.2/tags#if).
