	...
	public static void verificaTipos(Long idSolicitud) {
		...
		if (permisoverificaTipos("update") || permisoverificaTipos("create")) {
			...
			try {
				...
				CustomValidation.error("Error al recuperar los documentos nuevos a verificar", null, null);
			}
			
			if (dbSolicitud.verificacion.documentos == null) {
				CustomValidation.error("Error al recuperar los documentos nuevos a verificar", null, null);
				play.Logger.error("Error al recuperar los documentos nuevos a verificar: La lista de documentos a verificar es NULL");
			}
			
			if (!validation.hasErrors()) {
				play.Logger.info("No errores de validación aquí");
