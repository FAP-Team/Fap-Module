...
* **cascadeType = _[string]_** (opcional) : En atributos que sean referencias a otras entidades permite definir qué operaciones se harán en cascada.
...
bc. "String" | "Long" | "Integer" | "Boolean" | "Double" | "LongText" | "boolean"

Al definirnos un atributo con uno de estos tipos, se crea en la entidad un atributo con ese nombre y con este tipo. Ejemplo:
...
Para validar un atributo utilizando una expresión regular se utiliza la anotación 
*validate="RegExp"*, esta anotación solo es válida para atributos de tipo String y LongText. Para sobreescribir el mensaje de error de la validación modifique el valor de la clave *validation.match* en el fichero de mensajes.
...
*NOTA:* la cadena vacía se considera válida.
...
*NOTA:* El valor null se considera válido.
...
*Nota:* Sólo se puede validar si el valor es true. Si el valor es null se considera false.
