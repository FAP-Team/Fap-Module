...
// Ejecuta el doJob de la clase, cada X horas y al iniciar la aplicación
@OnApplicationStart
@Every("1h")
public class CompressLogs extends Job {
	
	static Integer tiempoRefresco = 0; // Para que se ejecute al @OnApplicationStart
	
	public void doJob() {
		if (AdministracionFapJobs.all() != null) {
			AdministracionFapJobs job = AdministracionFapJobs.all().first();
			if (job.comprimirLogs) {
				if ((tiempoRefresco == 0) || ((job.valorComprimirLogs != null) && (tiempoRefresco == job.valorComprimirLogs))){
					tiempoRefresco=1;
