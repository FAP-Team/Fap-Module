...
|_. Properties |_. Ejemplo |
| Configurar los logs: *app.log.path=* _Ruta del log4j de configuración_ | _app.log.path=/log4j-logsApp.properties_ |
| Configurar la activación de almacenar los log en la carpeta que se indica en fap.compresslogs.path: *fap.compresslogs.properties=* _[true / false]_ |  _fap.compresslogs.properties=false_ |
| Configurar filtro por tipo MIMES: *fap.gestordocumental.mimes=* _Tipos mimes_ | _fap.gestordocumental.mimes=image/png, image/jpg, application/pdf_
| Configurar filtro por extensiones: *fap.gestordocumental.extensions=* _Extensiones_ | _fap.gestordocumental.extensions=pdf, png, jpg_
| Ruta del manual de la aplicación (A partir de /public/): *fap.manual.aplicacion=* _RUTA_ | _fap.manual.aplicacion=manuales/manualApp1.pdf_
| Pagina inicial de la aplicación: *fap.app.firstPage=* _Nombre pagina DSL_ | _fap.app.firstPage=DatosProyecto_
| Configurar el formato de los doubles: *fap.format.double=* _Formato_ | _fap.format.double=0.00_
| Configurar Login con Usuario y Contraseña: *fap.login.type.user=* _[true / false]_ | _fap.login.type.user=true_
| Configurar Tamaño mínimo de las contraseñas: *fap.password.min=* _Número_ | _fap.login.type.min=6_
| Configurar Tamaño máximo de las contraseñas: *fap.password.max=* _Número_ |  _fap.login.type.max=10_
| Nombre de la aplicación: *fap.app.name=* _NOMBRE_ |  _fap.app.name=Tueri_
...
| Configuración del borrado de los pdf temporales: *fap.delete.temporals=* _[true / false]_ | _fap.delete.temporals=true_ (Activa el borrado, por defecto es false)
| Vejez de los pdf temporales a borrar: *fap.delete.temporals.old=* _[Xd / Xh / Xm]_ | _fap.delete.temporals.old=3h_ (Borrar PDF temporales con más de 3 horas de antiguedad)
...
| Cada cuanto se debe ejecutar el Job encargado de actualizar las Notificaciones (en minutos) *fap.notificacion.refrescoBaseDeDatosFromWS=* _Número_ | _fap.notificacion.refrescoBaseDeDatosFromWS=1_ (Valor por defecto que trae el módulo FAP)
| Cada cuanto se debe ejecutar el Job encargado de gestionar lo del Seguimiento (en minutos) *fap.seguimiento.notificarAlertar.anotaciones=* _Número_ | _fap.seguimiento.notificarAlertar.anotaciones=1_ (Valor por defecto que trae el módulo FAP)
| Cada cuanto se debe ejecutar el Job encargado de comprimir los logs (en horas) *fap.log.compress.every=* _Número_ | _fap.log.compress.every=24_ (Valor por defecto que trae el módulo FAP)
| Cada cuanto se debe ejecutar el Job encargado de eliminar los archivos temporales antiguos (en horas) *fap.delete.temporals.every=* _Número_ | _fap.delete.temporals.every=5_ (Valor por defecto que trae el módulo FAP)
...
| Permitir que un Funcionario Habilitado firme. *fap.firmaYRegistro.funcionarioHabilitado*= _[true / false]_ | _fap.firmaYRegistro.funcionarioHabilitado=false_ (Valor por defecto que trae el mñodulo FAP)
