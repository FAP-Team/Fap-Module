...
public class PresentarModificacionFAPController extends PresentarModificacionFAPControllerGen {
	...
	public static void deshacer(Long idSolicitud, Long idRegistroModificacion, Long idRegistro, String botonModificar) {
		...
		if (!Messages.hasErrors()) {
			...
			try {
				...
				Messages.error("Hubo un problema al intentar deshacer la Presentación");
				play.Logger.info("No se ha podido deshacer la presentación de la solicitud: "+e.getMessage());
			}
		}
	...
	@Util
	// Este @Util es necesario porque en determinadas circunstancias crear(..) llama a editar(..).
	public static void frmPresentar(Long idSolicitud, Long idRegistroModificacion, Long idRegistro, String botonPresentar) {
		checkAuthenticity();
		if (!permisoFrmPresentar("editar")) {
			Messages.error("No tiene permisos suficientes para realizar la acción");
		}
		...
		if (!Messages.hasErrors()) {
			PresentarModificacionFAPController.frmPresentarValidateRules();
		}
		if (!Messages.hasErrors()) {
			...
			log.info("Acción Editar de página: " + "gen/PresentarModificacionFAP/PresentarModificacionFAP.html" + " , intentada sin éxito (Problemas de Validación)");
		PresentarModificacionFAPController.frmPresentarRender(idSolicitud, idRegistroModificacion, idRegistro);
	}
