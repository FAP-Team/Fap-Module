...
|_. Definición |_. Nombre |_. Valor |_. Ejemplo |
| Ruta para la subida de imágenes en el editor enriquecido para las plantillas (antiguos informes odt) | *fap.path.editor.img* | | _fap.path.editor.img=/public/images_ (Valor por defecto que trae el módulo FAP) |
|Ruta para la subida de ficheros temporales por el editor enriquecido para las plantillas (antiguos informes odt) | *fap.path.editor.tmp* | | _fap.path.editor.tmp=/public/tmp_ (Valor por defecto que trae el módulo FAP) |
| Si es true, la lista de entidades seleccionables en el plugin insertarentidadfap del editor será la de la constante ENTIDADES_A_INCLUIR declarada en PlantillasDocController. Si es false, se listan todas las entidades de la aplicación, menos las que aparecen en la lista constante ENTIDADES_A_EXCLUIR (PlantillasDocController) | *fap.editor.entidades.incluir* | _[true | false]_ | _fap.editor.entidades.incluir=true_ (Valor por defecto que trae el módulo FAP) |
