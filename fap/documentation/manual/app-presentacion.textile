...
Además de ver los métodos que implemente el Trámite Base y que podemos "sobreescribir":app-tramiteBase
...
* *5. Método para personalizar acciones a realizar despues de firmar.*

bc. public static void afterFirma(Long idSolicitud)

Esta función se utiliza para ejecutar código que sea necesario ejecutar despues del proceso de Firma en la Presentación. De esta manera, lo que en esta función pongamos, se ejecutará siempre justo despues de firmar las solicitudes. Puede ser, por ejemplo, indicar alguna condición que se debe dar para proseguir con el registro (en este caso debemos especificar en esta función mediante Messages.error(...) que la condicion no se cumple para que no se haga nada posteriormente), o cualquier otra cosa.

Por defecto en Fap, esta función no tiene nada, por lo que no se ejecuta ni comprueba nada especial despues de firmar.
...
* *7. Método para comprobar si se ha guardado las páginas asociadas a la fase que se va a Presentar.*
...
Para personalizar el documento generado en la presentación se deberá sobreescribir una serie de plantillas, que deberan ir en la ruta _app/views/reports/_ situada en la raiz de nuestra aplicación.
...
Si por el contrario queremos hacer uso de lo que ya trae definido FAP (en TramiteSolicitudFap), las funciones pertinentes de FAP, para la cabecera, el contenido y el pie de pagina de dicho documento son las siguientes:
...
* *footer-borrador.html*: Para el pie de pagina del documento borrador.
