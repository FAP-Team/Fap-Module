...
Una vez "configurado el entorno":curso-instalacion, abrimos una consola del sistema, nos situamos donde queramos poner físicamente la nueva aplicación y tecleamos:
...
Para este ejemplo vamos a utilizar el gestor de base de datos MySQL (si no la tiene instalada, volver al apartado de "configuración del entorno":curso-instalacion). Creamos una nueva base de datos para esta aplicación en nuestro propio equipo (en el ejemplo también la llamaremos _nuevaPaginaInicio_). Podemos usar un asistente gráfico o desde consola:
