...
* **dependeDe = _[combo]_** (opcional) : Indica que el combo es un combo dependiente del combo que se especifica, o sea que los valores que se mostrarán en dicho combo, para sus selección, dependerán del valor seleccionado en el combo del que depende.
* **ordenar = (ordenarPorClave | ordenarPorValor)** (opcional) : Permite ordenar los elementos del combo por el elemento especificado.
* **eliminarDuplicados** (opcional) : Los elementos del combo, nunca tendrán valores repetidos, por lo que se filtrarán si esto ocurriese.
...
El método definido en el controlador devuelve una lista de **ComboItem**. Es importante que la clave de los elementos ComboItem coincida con el tipo de dato definido en la entidad donde se va a guardar el dato. En caso contrario dará error.
...
!images/dsl-comboManual!

!images/dsl-comboMultiple!
...
!images/dsl-comboReferencia!

h3. Lista de elemento a partir de un servicio web (JSON o XML)

En este ejemplo para acceder a los servicios web se utilizan las librerías que provee PlayFramework y se hace un parseo manual de la respuesta que da el servicio. En el caso de que se esté accediendo a un servicio web SOAP es conveniente crear una librería para acceder al servicio de una manera más cómoda.
