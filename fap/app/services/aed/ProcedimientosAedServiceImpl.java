 ...
 * 
 */
public class ProcedimientosAedServiceImpl {

	private static Logger log = Logger.getLogger(ProcedimientosAedServiceImpl.class);
	...
	public ProcedimientosAedServiceImpl(PropertyPlaceholder propertyPlaceholder, TiposDocumentosService tiposDocumentosService) {
	    this.propertyPlaceholder = propertyPlaceholder;
        this.tiposDocumentosService = tiposDocumentosService;
        URL wsdlProcedimientosURL = Aed.class.getClassLoader().getResource ("wsdl/procedimientos/dbprocedimientos.wsdl");
	...
	/*
	 * Este método indica si existe el procedimiento que se ha declarado en la
	 * propertie fap.aed.procedimientos.procedimiento.uri
	 */
	
	public boolean consultarProcedimiento(){
		try {
			String uriProcedimiento = propertyPlaceholder.get("fap.aed.procedimientos.procedimiento.uri");
			ListaProcedimientos listaProc = procedimientosPort.consultarProcedimientos();
			if (listaProc != null) {
					List<Procedimiento> lista = listaProc.getProcedimientos();
					for (Procedimiento procedimiento : lista) {
						if (procedimiento.getUri().equals(uriProcedimiento)){
							return true;
						}
					}
			}
			...
			play.Logger.error("Se ha producido un error consultando el Procedimiento: "+FapProperties.get("fap.aed.procedimientos.procedimiento.uri"));
			e.printStackTrace();
			return false;
		}
	}
