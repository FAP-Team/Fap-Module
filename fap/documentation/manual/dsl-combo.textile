...
Permite definir un Combo para seleccionar un valor de una lista de valores. 

h2. Parámetros

* **name** (opcional) - Identificador del elemento
* **campo** - Atributo donde se almacenará la clave. Los tipos de atributos permitidos son:
** **Listas** - Cuando el atributo es de tipo lista, los elementos del desplegable mostrarán todos los elementos de la tabla.
** **Tipo simples** - Por ejemplo String o Long. Este caso está pensado para crear un método en el controlador manual donde se especifiquen los elementos que debe contener el desplegable.
** **Referencias** - Referencias a otras entidades. Las referencias pueden ser **ManyToOne** o **ManyToMany**. Cuando la referencia es de tipos **ManyToMany** se sobreentiende una lista múltiple. En el caso de las referencias es importante especificar el campo **comboValor** para mostrar la descripción adecuada, en caso contrario se mostrará el identificador.
* **titulo** (opcional) - Etiqueta que se mostrará al lado del combo
...
* **multiple** (opcional) - **Deprecated**. Actualmente, para comprobar si un combo es múltiple se comprueba si el tipo del campo es de tipo Collection (Listas multiples, referencias ManyToMany, Set o List de tipos básicos)
* **ancho** (opcional) - Especifica el ancho que tiene el campo
* **anchoTitulo** (opcional) - Permite indicar el ancho de la etiqueta de titulo.
* **comboTexto** (opcional) - Cuando
* **comboValor** (opcional)
...
bc.. Entidad ComboTest(Solicitud) {
...
Combo titulo="Lista" campo="solicitud.comboTest.list" requerido
Combo titulo="Lista" campo="solicitud.comboTest.listMultiple" requerido
